...
// // {
// //     public static string Generate(AstroProgram program)
// //     {
...
// //         {
// //             sb.AppendLine($"  {arg.Name} : {arg.TypeId} ({arg.Direction})" +
...
// //         {
// //             sb.AppendLine($"  Type: {kv.Key}");
// //             foreach (var v in kv.Value.Variables)
// //                 sb.AppendLine($"    {v.Name} = {v.Value?.ToString() ?? "<null>"}" +
// //                               (v.Comment != null ? $" // {v.Comment}" : ""));
// //         }

// //         sb.AppendLine("\nLabels:");
// //         foreach (var lbl in program.Labels)
// //             sb.AppendLine($"  {lbl.Key} -> line {lbl.Value}");

// //         sb.AppendLine("\nInstructions:");
// //         foreach (var instr in program.Lines)
// //         {
// //             sb.Append($"  [{instr.LineNumber}] ");
// //             if (instr.Breakpoint) sb.Append("* ");
// //             sb.Append($"{instr.FormId}");
...
// //             if (instr.Fields.Any())
// //             {
// //                 foreach (var field in instr.Fields)
// //                 {
// //                     var valueStr = field.Value switch
// //                     {
// //                         ConstantFieldValue c => c.Value?.ToString() ?? "<null>",
// //                         VariableFieldValue v => $"{v.TableSetName}.{v.VariableName}",
// //                         ExpressionFieldValue e => $"\"{e.Expression}\"",
// //                         EnumFieldValue e => e.SelectedValue,
// //                         FunctionCallFieldValue f => $"{f.FunctionName}(...)",
// //                         LabelFieldValue l => $":{l.LabelName}",
// //                         _ => field.Value?.ToString() ?? "<null>"
// //                     };
// //                     sb.AppendLine($"      {field.Key} = {valueStr}");
// //                 }
// //             }
// //         }

// //         sb.AppendLine("\nPermissions:");
// //         sb.AppendLine($"  Read: {string.Join(", ", program.Permissions.ReadRoles)}");
// //         sb.AppendLine($"  Write: {string.Join(", ", program.Permissions.WriteRoles)}");
// //         sb.AppendLine($"  Execute: {string.Join(", ", program.Permissions.ExecuteRoles)}");
...
//     private const string Separator = "─".PadRight(80, '─');
